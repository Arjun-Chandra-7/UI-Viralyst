...
const OUT_DIR = path.resolve('visual-results/reference');
fs.mkdirSync(OUT_DIR, { recursive: true });

(async () => {
  console.log('=== CAPTURING REFERENCE CHECKPOINTS ===');
  const browser = await chromium.launch({ headless: true });

  for (const vp of VIEWPORTS) {
    console.log(`\n--- Capturing reference at viewport: ${vp.name} (${vp.width}x${vp.height}) ---`);
    ...
    if (vp.name === 'desktop_1440') {
      ...
      await page.goto('https://www.aardvarkbookclub.com', { waitUntil: 'domcontentloaded' });
  ...
  console.log('Reference capture complete. All files saved to visual-results/reference/');
  await browser.close();
})();
